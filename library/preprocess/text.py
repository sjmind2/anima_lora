...
def _cache_text_embeddings_subset(
    image_dir: Path,
    tokenize_strategy,
    encoding_strategy,
    text_encoder,
    *,
    ...
    for batch_start in range(0, len(entries), batch_size):
        ...
        for img_path, caption in batch:
            cache_path = _te_cache_path(img_path, cache_dir, image_dir)
...
def cache_text_embeddings(
    ...
    *,
    ...
    When ``tree=True``, direct subdirectories of ``data_dir`` are treated as
    independent subsets. For each ``<sub>``: images are read from
    ``<sub>/.resized/`` (falling back to ``<sub>/``), and TE caches are written
    to ``cache_dir/<sub>/.lora/`` (or ``data_dir/<sub>/.lora/`` when
    ``cache_dir`` is None). Root-level images under ``data_dir/.resized/`` are
    also processed, with caches written to ``cache_dir/.lora/``.
    """
    if not tree:
        return _cache_text_embeddings_subset(
            data_dir,
            tokenize_strategy,
            encoding_strategy,
            text_encoder,
    ...
    subsets: list[tuple[Path, Path]] = []

    for child in sorted(data_dir.iterdir()):
        if not child.is_dir() or child.name.startswith("."):
            continue
        resized = child / ".resized"
        subset_image_dir = resized if resized.is_dir() else child
        subset_cache = (
            cache_dir / child.name / ".lora"
            if cache_dir is not None
            else child / ".lora"
        )
        subsets.append((subset_image_dir, subset_cache))

    root_resized = data_dir / ".resized"
    if root_resized.is_dir():
        root_cache = (
            cache_dir / ".lora" if cache_dir is not None else data_dir / ".lora"
        )
        subsets.append((root_resized, root_cache))

    stats = PreprocessStats()
    for subset_image_dir, subset_cache_dir in subsets:
        if not subset_image_dir.is_dir():
            continue
        subset_stats = _cache_text_embeddings_subset(
            subset_image_dir,
            tokenize_strategy,
            encoding_strategy,
            text_encoder,
        ...
        stats.seen += subset_stats.seen
        stats.written += subset_stats.written
        stats.skipped += subset_stats.skipped
        stats.failed += subset_stats.failed
