...
def resize_to_buckets(
    ...
    *,
    ...
    if tree:
        ...
        subsets = []
        root_images = walk_images(src, recursive=False)
        if root_images:
        ...
        total_buckets: dict[tuple[int, int], int] = {}

        for name, subset_src, subset_dst in subsets:
            image_files = walk_images(subset_src, recursive=False)
            subset_stats = PreprocessStats(seen=len(image_files))
            ...
            if min_pixels > 0:
                ...
                subset_stats.skipped = len(skipped)
                image_files = kept
            ...
            with ProcessPoolExecutor(max_workers=workers) as pool:
                ...
                for future in as_completed(futures):
                    ...
                    if progress is not None:
                        progress(1, detail=f"{fname} → {reso[0]}x{reso[1]}")
        ...
        if verbose:
            print("\nBucket distribution:")
            for reso in sorted(total_buckets):
                tokens = (reso[0] // 16) * (reso[1] // 16)
                print(
                    f"  {reso[0]:>4d}x{reso[1]:<4d}: {total_buckets[reso]:>3d} "
                    f"images  ({tokens} tokens)"
                )

        return total_stats, total_buckets
