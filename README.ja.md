...
> **重要：LoRA / LyCORIS の学習には必ず正則化（reg）データセットを使用してください。** reg データなしで学習すると、ベースモデルの能力が深刻に汚染される可能性があります — アダプタが学習画像に過学習し、モデルの汎化性能が低下します（スタイルの混入、ポーズの固定、品質の低下など）。ベースモデルの元の性能を維持するため、多様な汎用画像を reg データセットとして含めてください。これは Anima の DiT アーキテクチャにおいて特に重要です。
...
### バケットファミリ — ディテール精度と学習速度のトレードオフ

Anima の `CONSTANT_TOKEN_BUCKETS` テーブルは解像度を **トークンカウント (TC) ファミリ** でグループ化します — 各ファミリは固定のパッチグリッド面積を持ち、`torch.compile` はファミリごとに1つのブロックグラフのみをトレースします。TC ファミリが小さいほど学習が高速（ステップあたりのパッチが少ない）ですが、ディテールが犠牲になります。大きなファミリはより多くの空間情報を保持しますが、計算量と VRAM が増加します。

| ファミリ | トークンカウント | 最大解像度 | 用途 |
|---|---|---|---|
| XS | 1680 | 640×672 | 最速の反復、低ディテール（顔/ポーズ実験） |
| S | 2160 | 720×768 | 高速ドラフト、小規模データセット |
| M | 3600 | 960×960 | バランスの取れた品質/速度、ほとんどのキャラクター向け |
| L | 4032 | 1008×1024 | 高ディテール、ほぼ正方形のアスペクト比 |
| S1 | 1024 | 512×512 | 小さな正方形、極限の低 VRAM |
| S2 | 4096 | 1024×1024 | フル 1MP 正方形、最大ディテール |
| XL | 5040 | 1120×1152 | 最大ディテール、最も広いアスペクト比範囲 |

設定の `bucket_families` で選択（例：`bucket_families = ["S2"]` で 1024² のみの学習）。異なる TC グループのファミリを混在可能 — TC ファミリが増えるごとに1つのコンパイルブロックグラフが追加されます。最適なコンパイルキャッシュヒット率のために、単一の TC ファミリを使用することを推奨します。

### 勾配チェックポイント — VRAM と速度のトレードオフ

勾配チェックポイント (GC) は計算を VRAM と引き換えます：フォワードの全アクティベーションをバックワード用に保持する代わりに、破棄してバックワード時に再計算します。3つのノブでこのトレードオフを制御します：

| 設定 | VRAM | 速度 | 用途 |
|------|------|------|------|
| GC オフ | 1024² bs=4 約 32 GB | 最速（再計算なし） | 24 GB+ メモリ余裕のある GPU |
| フル GC (`last_n=0`) | 約 8–10 GB | 約 1.7 倍遅い（全28ブロック再計算） | VRAM が厳しい、大バッチや高解像度 |
| 選択的 GC (`last_n=N`) | N に応じて変動（チェックポイントが少ない = VRAM 増） | フル GC より高速（再計算ブロックが少ない） | 速度と VRAM ヘッドルームのバランス |
| **GC + SAC** | フル GC + 約 2–4 GB 追加 | 標準 GC より 15–25% 高速 | **推奨** GC 有効学習向け |

**SAC（選択的アクティベーションチェックポイント）** は PyTorch 2.12 の `CheckpointPolicy` を使用し、高コストの attention 演算（flash attention、softmax）を保存し、バックワード時に安価な演算（LayerNorm、要素ごと、線形射影）のみを再計算します。これにより再計算の最も高コストな部分を排除しつつ、アクティベーションメモリの増加を控えめに抑えます。設定で `gradient_checkpointing_sac = true` を有効化（`torch_compile = true` が必要）。

**`last_n` パラメータ** — 0 より大きい値に設定すると、最後の N 個の DiT ブロック（出力層に最も近い）のみがチェックポイントを使用し、残りのブロックは通常通り実行されます。`last_n=0` は「全28ブロックをチェックポイント」（最大 VRAM 節約）を意味するため、最も積極的な GC 設定です。`last_n` を減らす（例：20〜22）と、早期ブロックをチェックポイントから外すことで段階的に VRAM を速度と引き換えます。最適値は VRAM 予算に依存 — 高めの値（例：`last_n=22`）から開始し、OOM が発生した場合のみ増やしてください。

### パフォーマンスチューニング — GPU 使用率とベースライン

小さな TC ファミリはハイエンド GPU の使用率が低くなります。例えば RTX 5090 では、S1 ファミリ（TC=1024）は `bs=4` で GPU 使用率 62〜69% のみ維持 — 計算量が軽すぎて GPU を飽和させられません。この場合、**バッチサイズの増加** が最も効果的なレバーです：サンプルあたりの計算量を変えずに GPU 使用率を向上させます。ただし、より大きなバッチはより多くの VRAM を消費するため、GPU を完全に活用しつつオーバーヘッドを最小限に抑えるために、`batch_size` / GC / SAC / `last_n` を総合的にバランスさせる必要があります。

**パフォーマンスベースライン**（RTX 5090、LoKR dim/alpha=16/8 factor=8、CAME_C）：

| 設定 | TC | bs | 秒/ステップ | 秒/サンプル | 備考 |
|------|-----|-----|-------------|-------------|------|
| S1 | 1024 | 4 | ~0.4 | ~0.10 | この TC の理論限界に近い |
| S2 | 4096 | 1 | ~0.42 | ~0.42 | 計算量は線形スケール：4096/1024 ≈ 0.42/0.10 |

S1 を `bs=4` で実行すると、1ステップあたり4サンプルを ~0.4 秒で処理し、~0.10 秒/サンプル — 実質的に計算の下限に到達。S2 を `bs=1` で実行すると、1ステップあたり1サンプルをトークン数4倍で処理するため、サンプルあたりの時間は ~0.42 秒に上昇し、4096/1024 の計算量比と一致します。どちらもそれぞれの性能上限に近い値です。

**CAME_C と fused AdamW** — CAME_C は fused AdamW と比較してステップあたりミリ秒レベルのオーバーヘッドがありますが、~0.1 秒/ステップ以上では無視できます（ステップ時間の 1% 未満）。

**数値精度** — CAME/CAME_C は 15k ステップ後に ~1e-5 の累積誤差が生じる可能性があります。SAC の有効化はさらに ~1e-5 の誤差を追加します。いずれも LoRA 学習の許容範囲内であり、最終的なモデル品質への実用上の影響は限定的です。
