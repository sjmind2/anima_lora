...
Wired through `LoRAModule`, `HydraLoRAModule`, `OrthoLoRAExpModule`, `OrthoHydraLoRAExpModule`, and `ChimeraHydraLoRAExpModule`. The Ortho/Chimera variants pass `Q_eff = R_q @ Q_basis` as the "weight" argument — autograd returns `grad_Q_eff`, which the existing graph propagates into `S_q` unchanged; Chimera issues two such calls (one per pool) on a shared rebalanced `x_in`, deduping the saved-for-backward input. ReFT (block-level intervention) and Conv2d LoRA are intentionally out of scope and take the legacy path.
