...
> **중요: LoRA / LyCORIS 학습 시 반드시 정규화(reg) 데이터셋을 사용하세요.** reg 데이터 없이 학습하면 베이스 모델의 능력이 심각하게 오염될 수 있습니다 — 어댑터가 학습 이미지에 과적합되어 모델의 일반화 성능이 저하됩니다 (예: 스타일 유출, 포즈 고정, 품질 저하). 베이스 모델의 원래 성능을 유지하려면 다양한 범용 이미지를 reg 데이터셋으로 포함하세요. 이는 Anima의 DiT 아키텍처에서 특히 중요합니다.
