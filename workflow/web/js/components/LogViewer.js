(function () {
  ...
  window.VueComponents.LogViewer = {
    ...
    template: [
      ...
      '    <input type="text" class="form-input lv-search" :placeholder="t(\'logViewer.searchPlaceholder\')"',
      '      v-model="searchText" />',
      '    <select class="form-select lv-stage-filter" v-model="stageFilter">',
      '      <option value="">{{ t(\'logViewer.allStages\') }}</option>',
      '      <option v-for="s in stages" :key="s" :value="s">{{ s }}</option>',
      '    </select>',
      '    <button v-if="paused" class="btn btn-ghost btn-sm" @click="resumeScroll">',
      '      {{ t(\'logViewer.resume\') }}',
      '    </button>',
      '    <span v-if="paused" class="lv-paused-badge">{{ t(\'logViewer.paused\') }}</span>',
      '  </div>',
